...
| Функциональное (FR-04) | Fallback на человека | При уверенности модели < 0.50 получить ответ опользователя о рекомендации | 
| Нефункциональное (NFR-01) |Производительность | Время отклика API не должно превышать 200 мс | 
| Нефункциональное (NFR-03) | Воспроизводимость | Каждый ответ модели содержит метаданные: model_version и request_id. | 
| Нефункциональное (NFR-03) | Безопасность | Персональные данные пользователей анонимизированы. | 

### Rонтекстная диаграмма
flowchart LR
...
| :--- | :--- | :--- | :--- |
| Пользователь → Система | HTTPS (REST) | По запросу | JSON: `{ "user_id": "...", "context": {...} }` |
| Система → Пользователь | HTTPS (REST) | По запросу | JSON: `{ "prediction": "...", "confidence": 0.67, "recommendations": [...] }` |
| Система → Корпоративная БД | SQL / REST | При каждом запросе | SQL-запрос |
| Система → Сервис уведомлений | HTTPS (Webhook) | Только при критических сбоях | JSON: `{ "level": "critical", "message": "...", "timestamp": "..." }` |
...
- **Рассмотренные альтернативы:**
  1. **Synchronous REST API (FastAPI + Uvicorn)** — клиент отправляет запрос и ждёт ответа.
  2. **Asynchronous Message Queue (RabbitMQ / Kafka)** — клиент публикует задачу, сервис обрабатывает её в фоне и возвращает результат через callback или polling.
- **Решение:** 
...
- **Рассмотренные альтернативы:**
  1. **Микросервисная архитектура** — каждый модуль отдельный контейнер, общаются по HTTP.
  2. **Модульный монолит** — все модули в одном Python-приложении, но разделены по пакетам.
- **Решение:** 
